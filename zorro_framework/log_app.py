...
class LogApplication:
    ...
    def __call__(self, environ, start_response):
        ...
        if method == 'POST':
            ...
            request['data'] = LogApplication.decode_value(data)
            logger.log(f'METHOD: {method} :: PARAMETERS: {LogApplication.decode_value(data)}')
        elif method == 'GET':
            request_params = GetRequests().get_request_params(environ)
            request['request_params'] = request_params
            logger.log(f'METHOD: {method} :: PARAMETERS: {request_params}')
