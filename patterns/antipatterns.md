...
В функции `get_course_by_name` в модуле `db_mappers` есть строчка, где захардкожены результаты запроса. То есть, если порядок в изначальном запросе поменяется, то и последующая функция сломается. Возможно здесь лучше использовать **namedTuple**.
