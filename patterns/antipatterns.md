...
Например, функцию `check_length`, которая бы проверяла длину вводимых данных. Функция бы вызывалась в `views` для полей, где необходима эта проверка. Это будет выглядеть приблизительно так:
